...
def main() -> None:
    ...
    set_text(
        basis[5],
        "本课题以红壤丘陵区设施农业为应用场景，拟设计基于物联网与协同控制的智能水肥一体化管控系统。系统由双EC传感与设备状态采集、注肥执行机构、边缘控制节点、MQTT通信、云端控制服务、数据存储和管理界面组成；控制层采用风险感知自适应边云监督控制（RA-ECSC），在M0云端预测模式、M1边缘控制模式和M2模型锚定降级模式之间分配控制权。三种模式共享底层PI、饱和、抗积分饱和和动作变化率约束，监督器负责风险计算、模式转换、状态同步与恢复管理。",
    )
    set_text(
        basis[8],
