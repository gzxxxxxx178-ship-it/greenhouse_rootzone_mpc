...
from rootzone_mpc.design.supervision_v2_manifest import (
    build_supervision_v2_manifests,
    build_supervision_v3_manifests,
)
...
def test_v3_manifest_uses_distinct_seed_space(tmp_path: Path):
    root = Path(__file__).resolve().parents[1]
    (tmp_path / "configs").mkdir()
    (tmp_path / "data/processed").mkdir(parents=True)
    for name in ["supervision_v3_design.yaml", "scenario_design_v1.yaml", "anomaly_design_v1.yaml"]:
        (tmp_path / "configs" / name).write_bytes((root / "configs" / name).read_bytes())
    scenario_path, _, _ = build_supervision_v3_manifests(tmp_path)
    scenarios = pd.read_csv(scenario_path)
    assert scenarios.scenario_id.str.startswith("V3").all()
    assert scenarios.scenario_seed.is_unique
